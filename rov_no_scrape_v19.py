...
def load_metadata():
    ...
    for asn, ccs in asn_countries.items():
        ...
        # If ASN exists in meta, update it. 
        # If not, create it (we found an ASN that BGP.tools missed?)
        if asn in meta:
            meta[asn]['cc'] = primary_cc
            updates += 1
        else:
            meta[asn] = {'name': 'Unknown', 'cc': primary_cc}
            updates += 1
            
    print(f"    - Geo-tagged {updates} ASNs across {len(unique_ccs)} countries.")
...
def sync_apnic_data(countries):
    ...
    print(f"    - Sync Complete: {updated} Fetched, {cached} Cached.")
...
def analyze():
    ...
    # Report
    print("\n" + "="*80)
    print("NO-SCRAPE GLOBAL AUDIT (V19 - DUAL STACK GEO)")
    print("="*80)
    ...
    filename = "rov_audit_v19_final.csv"
