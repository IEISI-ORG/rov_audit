    [*] Loading Global Audit for PT...

    ====================================================================================================
     NATIONAL ROUTING SECURITY: PT
    ====================================================================================================
    Total Networks:      163
    Total Cone Gravity:  277
    ------------------------------------------------------------
    SECURE (ACTIVE/PASSIVE):    66 (40.5%) -> Protects 24.9% of Traffic
    VULNERABLE NETWORKS:        58 (35.6%) -> Exposes  12.6% of Traffic

    ====================================================================================================
     THE PT CORE (Top 20 Networks)
    ====================================================================================================
    ASN      | Verdict                        | Cone     | APNIC% | Name
    ----------------------------------------------------------------------------------------------------
    AS6424   | PARTIAL: VULNERABLE (Mixed)    | 146      | -      | EDGOO NETWORKS
    AS2860   | VOLATILE                       | 43       | 99%    | NOS COMUNICACOES, S.A.
    AS15525  | Unverified (Transit/Peer?)     | 23       | 2%     | MEO - SERVICOS DE COMUNICACOES E MULTIME
    AS8657   | REGRESSED                      | 22       | -      | MEO International Backbone
    AS9186   | REGRESSED                      | 11       | 1%     | ONITELECOM - INFOCOMUNICACOES, S.A.
    AS12353  | VOLATILE                       | 9        | 99%    | Vodafone Portugal - Communicacoes Pessoa
    AS3243   | VOLATILE                       | 7        | 0%     | MEO
    AS1930   | ACTIVE LOCAL ROV               | 4        | 99%    | Fundacao para a Ciencia e a Tecnologia, 
    AS24768  | ACTIVE LOCAL ROV               | 3        | 0%     | AlmourolTec, Lda
    AS29003  | PASSIVE (Clean Pipe)           | 2        | 100%   | IP TELECOM, SERVICOS DE TELECOMUNICACOES
    AS209874 | VULNERABLE                     | 2        | 0%     | Tech Tide Portugal Unipessoal LDA
    AS47787  | PARTIAL: VULNERABLE (Mixed)    | 2        | -      | EDGOO NETWORKS UNIPESSOAL LDA
    AS201782 | PASSIVE (Clean Pipe)           | 1        | -      | Make It Simple Consultoria Informatica L
    AS200454 | PARTIAL: VULNERABLE (Mixed)    | 1        | -      | Tomas Oliveira Valente Leite de Castro
    AS50293  | PARTIAL: VULNERABLE (Mixed)    | 1        | -      | Interfiber Networks LDA
    AS199155 | STUB: FORTUITOUS ROV           | 0        | 99%    | Direcao Geral de Estatisticas da Educaca
    AS198980 | NOT ROUTED                     | 0        | -      | Banco de Portugal
    AS199016 | STUB: VULNERABLE               | 0        | -      | VISUALFORMA TECNOLOGIAS DE INFORMACAO, S
    AS199130 | STUB: PASSIVE (Clean Pipe)     | 0        | -      | 4Spiro - Sociedade de Consultoria, LDA
    AS199667 | NOT ROUTED                     | 0        | -      | 4Spiro - Sociedade de Consultoria, LDA

    ====================================================================================================
     TRANSIT SUPPLY CHAIN (Who provides to PT?)
    ====================================================================================================
    Rank | Upstream | Dependents | Global Status                  | Name
    ----------------------------------------------------------------------------------------------------
    #1   | AS2860   | 34         | VOLATILE                       | NOS COMUNICACOES, S.A.
    #2   | AS15525  | 20         | Unverified (Transit/Peer?)     | MEO - SERVICOS DE COMUNICACOES E MULTIME
    #3   | AS174    | 18         | CORE: ACTIVE PROTECTOR         | Cogent Communications, LLC
    #4   | AS12353  | 11         | VOLATILE                       | Vodafone Portugal - Communicacoes Pessoa
    #5   | AS6939   | 9          | CORE: ACTIVE PROTECTOR         | Hurricane Electric LLC
    #6   | AS6424   | 9          | PARTIAL: VULNERABLE (Mixed)    | EDGOO NETWORKS
    #7   | AS8220   | 9          | PARTIAL: VULNERABLE (Mixed)    | COLT
    #8   | AS1930   | 7          | ACTIVE LOCAL ROV               | Fundacao para a Ciencia e a Tecnologia, 
    #9   | AS9186   | 7          | REGRESSED                      | ONITELECOM - INFOCOMUNICACOES, S.A.
    #10  | AS3243   | 7          | VOLATILE                       | MEO
    #11  | AS29003  | 5          | PASSIVE (Clean Pipe)           | IP TELECOM, SERVICOS DE TELECOMUNICACOES
    #12  | AS24768  | 4          | ACTIVE LOCAL ROV               | AlmourolTec, Lda
    #13  | AS20473  | 4          | ACTIVE LOCAL ROV               | The Constant Company, LLC
    #14  | AS2914   | 3          | CORE: ACTIVE PROTECTOR         | NTT America, Inc.
    #15  | AS6762   | 3          | CORE: UNPROTECTED              | Telecom Italia Sparkle (Seabone)
    #16  | AS1299   | 3          | CORE: ACTIVE PROTECTOR         | Arelion (fka. Telia Carrier)
    #17  | AS8657   | 3          | REGRESSED                      | MEO International Backbone
    #18  | AS8426   | 3          | PARTIAL: VULNERABLE (Mixed)    | Claranet Limited
    #19  | AS6453   | 2          | CORE: ACTIVE PROTECTOR         | TATA Communications (America) Inc
    #20  | AS3257   | 2          | CORE: ACTIVE PROTECTOR         | GTT Communications Inc.
