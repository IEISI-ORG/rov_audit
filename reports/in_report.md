    [*] Loading Global Audit for IN...

    ====================================================================================================
     NATIONAL ROUTING SECURITY: IN
    ====================================================================================================
    Total Networks:      6,168
    Total Cone Gravity:  13,796
    ------------------------------------------------------------
    SECURE (ACTIVE/PASSIVE):   199 ( 3.2%) -> Protects 73.1% of Traffic
    VULNERABLE NETWORKS:      2561 (41.5%) -> Exposes  18.1% of Traffic

    ====================================================================================================
     THE IN CORE (Top 20 Networks)
    ====================================================================================================
    ASN      | Verdict                        | Cone     | APNIC% | Name
    ----------------------------------------------------------------------------------------------------
    AS9498   | PASSIVE (Clean Pipe)           | 7002     | 32%    | Bharti Airtel Ltd.
    AS4755   | PASSIVE (Clean Pipe)           | 2522     | 38%    | TATA Communications (formerly VSNL)
    AS9583   | PASSIVE (Clean Pipe)           | 533      | 37%    | Sify Limited
    AS18229  | VULNERABLE                     | 448      | 34%    | CtrlS
    AS55836  | Unverified (Transit/Peer?)     | 395      | 29%    | Reliance Jio Infocomm Limited
    AS55410  | PARTIAL: VULNERABLE (Mixed)    | 370      | 42%    | Vodafone Idea Ltd
    AS45820  | VULNERABLE                     | 336      | 39%    | Tata Teleservices ISP
    AS9730   | VULNERABLE                     | 230      | -      | Bharti Telesonic Ltd
    AS17665  | VULNERABLE                     | 134      | 37%    | ONEOTT INTERTAINMENT LIMITED
    AS17762  | VULNERABLE                     | 121      | 38%    | Tata Teleservices Maharashtra Ltd
    AS45117  | VULNERABLE                     | 103      | 27%    | Ishan Netsol Pvt Ltd
    AS55824  | Unverified (Transit/Peer?)     | 86       | 32%    | NKN Core Network
    AS17439  | VULNERABLE                     | 83       | -      | NTT Communications India Pvt. Ltd.
    AS9829   | PARTIAL: VULNERABLE (Mixed)    | 78       | 30%    | BSNL (Bharat Sanchar Nigam Ltd)
    AS133296 | VULNERABLE                     | 74       | -      | Web Werks India Pvt. Ltd.
    AS132770 | VULNERABLE                     | 64       | 37%    | Gazon Communications India Limited
    AS135718 | VULNERABLE                     | 55       | 36%    | DISHAWAVES INFONET PVT. LTD
    AS137085 | VULNERABLE                     | 51       | 36%    | ANONET COMMUNICATIONS PVT LTD
    AS136334 | VULNERABLE                     | 49       | 39%    | Vortex Netsol Private Limited
    AS140641 | PARTIAL: VULNERABLE (Mixed)    | 39       | -      | YOTTA NETWORK SERVICES PRIVATE LIMITED

    ====================================================================================================
     TRANSIT SUPPLY CHAIN (Who provides to IN?)
    ====================================================================================================
    Rank | Upstream | Dependents | Global Status                  | Name
    ----------------------------------------------------------------------------------------------------
    #1   | AS9498   | 682        | PASSIVE (Clean Pipe)           | Bharti Airtel Ltd.
    #2   | AS4755   | 561        | PASSIVE (Clean Pipe)           | TATA Communications (formerly VSNL)
    #3   | AS45820  | 186        | VULNERABLE                     | Tata Teleservices ISP
    #4   | AS9730   | 170        | VULNERABLE                     | Bharti Telesonic Ltd
    #5   | AS9583   | 167        | PASSIVE (Clean Pipe)           | Sify Limited
    #6   | AS55410  | 156        | PARTIAL: VULNERABLE (Mixed)    | Vodafone Idea Ltd
    #7   | AS55836  | 143        | Unverified (Transit/Peer?)     | Reliance Jio Infocomm Limited
    #8   | AS18229  | 139        | VULNERABLE                     | CtrlS
    #9   | AS45117  | 79         | VULNERABLE                     | Ishan Netsol Pvt Ltd
    #10  | AS55824  | 78         | Unverified (Transit/Peer?)     | NKN Core Network
    #11  | AS17762  | 76         | VULNERABLE                     | Tata Teleservices Maharashtra Ltd
    #12  | AS9829   | 71         | PARTIAL: VULNERABLE (Mixed)    | BSNL (Bharat Sanchar Nigam Ltd)
    #13  | AS133296 | 48         | VULNERABLE                     | Web Werks India Pvt. Ltd.
    #14  | AS17439  | 48         | VULNERABLE                     | NTT Communications India Pvt. Ltd.
    #15  | AS132770 | 48         | VULNERABLE                     | Gazon Communications India Limited
    #16  | AS17665  | 47         | VULNERABLE                     | ONEOTT INTERTAINMENT LIMITED
    #17  | AS136334 | 40         | VULNERABLE                     | Vortex Netsol Private Limited
    #18  | AS140641 | 38         | PARTIAL: VULNERABLE (Mixed)    | YOTTA NETWORK SERVICES PRIVATE LIMITED
    #19  | AS132215 | 38         | VULNERABLE                     | Powergrid Teleservices Limited
    #20  | AS135718 | 37         | VULNERABLE                     | DISHAWAVES INFONET PVT. LTD
