    [*] Loading Global Audit for CN...

    ====================================================================================================
     NATIONAL ROUTING SECURITY: CN
    ====================================================================================================
    Total Networks:      6,462
    Total Cone Gravity:  5,780
    ------------------------------------------------------------
    SECURE (ACTIVE/PASSIVE):   117 ( 1.8%) -> Protects 4.5% of Traffic
    VULNERABLE NETWORKS:      4715 (73.0%) -> Exposes  94.9% of Traffic

    ====================================================================================================
     THE CN CORE (Top 20 Networks)
    ====================================================================================================
    ASN      | Verdict                        | Cone     | APNIC% | Name
    ----------------------------------------------------------------------------------------------------
    AS38255  | VULNERABLE STABLE              | 4182     | -      | China Education and Research Network (CE
    AS4134   | CORE: UNPROTECTED              | 368      | 0%     | China Telecom Backbone
    AS24429  | PARTIAL: VULNERABLE (Mixed)    | 179      | -      | Alibaba Cloud
    AS4837   | CORE: UNPROTECTED              | 118      | 1%     | China Unicom Backbone
    AS4808   | ACTIVE (Atlas Verified)        | 80       | 1%     | China Unicom Beijing Province Network
    AS139317 | PARTIAL: VULNERABLE (Mixed)    | 72       | -      | Ningbo Dahuamao Information Technology C
    AS4847   | PARTIAL: VULNERABLE (Mixed)    | 68       | 0%     | China Telecom Beijing Province Network
    AS9808   | CORE: UNPROTECTED              | 61       | 0%     | China Mobile Backbone
    AS4809   | CORE: ACTIVE PROTECTOR         | 50       | 1%     | China Telecom Next Generation Carrier Ne
    AS56048  | ACTIVE (Atlas Verified)        | 49       | 1%     | China Mobile Group Beijing Company
    AS4812   | ACTIVE (Atlas Verified)        | 45       | 0%     | China Telecom Shanghai Province Network
    AS9425   | VULNERABLE STABLE              | 40       | -      | Future Internet Technology Infrastructur
    AS38272  | VULNERABLE STABLE              | 39       | -      | China Education and Research Network (CE
    AS23910  | VULNERABLE STABLE              | 35       | 3%     | China Next Generation Internet CERNET2
    AS17621  | VULNERABLE STABLE              | 33       | 2%     | China Unicom Shanghai network
    AS146788 | Unverified (Transit/Peer?)     | 32       | -      | China Broadcasting Network Co., Ltd
    AS24400  | ACTIVE (Atlas Verified)        | 31       | 0%     | Shanghai Mobile Communications Co.,Ltd.
    AS4538   | PARTIAL: VULNERABLE (Mixed)    | 28       | 2%     | China Education and Research Network Cen
    AS213605 | PARTIAL: VULNERABLE (Mixed)    | 24       | -      | Liu HaoRan
    AS56040  | VULNERABLE STABLE              | 18       | 0%     | China Mobile Group GuangDong Company

    ====================================================================================================
     TRANSIT SUPPLY CHAIN (Who provides to CN?)
    ====================================================================================================
    Rank | Upstream | Dependents | Global Status                  | Name
    ----------------------------------------------------------------------------------------------------
    #1   | AS38255  | 3859       | VULNERABLE STABLE              | China Education and Research Network (CE
    #2   | AS4134   | 142        | CORE: UNPROTECTED              | China Telecom Backbone
    #3   | AS20473  | 93         | ACTIVE (Atlas Verified)        | The Constant Company, LLC
    #4   | AS139317 | 69         | PARTIAL: VULNERABLE (Mixed)    | Ningbo Dahuamao Information Technology C
    #5   | AS4847   | 65         | PARTIAL: VULNERABLE (Mixed)    | China Telecom Beijing Province Network
    #6   | AS4808   | 62         | ACTIVE (Atlas Verified)        | China Unicom Beijing Province Network
    #7   | AS4837   | 55         | CORE: UNPROTECTED              | China Unicom Backbone
    #8   | AS38272  | 40         | VULNERABLE STABLE              | China Education and Research Network (CE
    #9   | AS9425   | 40         | VULNERABLE STABLE              | Future Internet Technology Infrastructur
    #10  | AS9808   | 36         | CORE: UNPROTECTED              | China Mobile Backbone
    #11  | AS4812   | 35         | ACTIVE (Atlas Verified)        | China Telecom Shanghai Province Network
    #12  | AS6939   | 33         | CORE: ACTIVE PROTECTOR         | Hurricane Electric LLC
    #13  | AS53667  | 33         | PARTIAL: VULNERABLE (Mixed)    | FranTech Solutions
    #14  | AS56048  | 33         | ACTIVE (Atlas Verified)        | China Mobile Group Beijing Company
    #15  | AS44324  | 31         | PARTIAL: VULNERABLE (Mixed)    | MoeDove LLC
    #16  | AS17621  | 29         | VULNERABLE STABLE              | China Unicom Shanghai network
    #17  | AS34927  | 27         | PARTIAL: VULNERABLE (Mixed)    | iFog GmbH
    #18  | AS24400  | 24         | ACTIVE (Atlas Verified)        | Shanghai Mobile Communications Co.,Ltd.
    #19  | AS7720   | 22         | PARTIAL: VULNERABLE (Mixed)    | Skywolf Technology LLC
    #20  | AS4809   | 21         | CORE: ACTIVE PROTECTOR         | China Telecom Next Generation Carrier Ne
