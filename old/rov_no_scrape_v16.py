...
# Constants
CACHE_TTL = 86400 # 24 Hours for APNIC data
        ...
        with gzip.open(BytesIO(resp.content), 'rt') as f:
            ...
            for asn, ccs in asn_countries.items():
                ...
                if asn in meta:
                    ...
                    updates += 1
            print(f"OK (Updated {updates} ASNs, Found {len(unique_ccs)} Countries)")
    except Exception as e:
        print(f"FAIL ({e})")
    ...
    for cc in countries:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for asn_str, val in matches:
                    scores[int(asn_str)] = float(val)
                ...
                # Save empty to prevent infinite retries on 404s
...
def calculate_cones(downstream_map):
    ...
    for i, asn in enumerate(downstream_map.keys()):
        if i % 10000 == 0: print(f"    - Progress {i}...", end="\r")
        cone_sizes[asn] = len(get_cone_set(asn))
    print("")
    return cone_sizes
