...
def analyze():
    ...
    # Filter for meaningful providers
    ...
    for idx, row in providers.iterrows():
        ...
        results.append({
            ...
            'CC': row['cc'],
            'Cone': row['cone'],
            'Percentile': avg_roa, # "Customer Signing %"
            'Name': row['name']
        })
    ...
    # --- REPORTING ---
    print("\n" + "="*110)
    print(f"ROV STRATEGIC QUADRANT REPORT (Top 5 per Category)")
    print("="*110)
    ...
    # Header
    print(f"{'Quadrant Name':<20} | {'ASN':<8} | {'CC':<2} | {'Cone':<8} | {'% Sign':<6} | {'Name'}")
    print("-" * 110)

    for q_name, color in q_order:
        # Filter and Sort by Cone Size
        subset = out_df[out_df['Quadrant'] == q_name].sort_values(by='Cone', ascending=False).head(5)
        
        for _, row in subset.iterrows():
            q_display = q_name.replace("Q", "Q") # Just to keep variable used
            
            print(f"{color}{q_display:<20}\033[0m | AS{row['ASN']:<6} | {row['CC']:<2} | {row['Cone']:<8} | {row['Percentile']:>5.1f}% | {row['Name'][:45]}")
        
        # Separator between quadrants
        print("-" * 110)

    # Save
    filename = "rov_quadrant_top5.csv"
