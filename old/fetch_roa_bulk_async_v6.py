...
# Tuning
MAX_WORKERS = 20
CACHE_TTL_DAYS = 7  # Increased Currency (Weekly refresh)
...
def aggregate_apnic_data(series):
    """Sum Valid/Total across all countries for the latest date."""
...
def fetch_and_update(asn):
    ...
    try:
        ...
        if not series: return "FAIL", "Empty"
        ...
        file_data['roa_raw_stats'] = {'valid': valid, 'total': total}
...
def main():
    ...
    print(f"[*] Starting 7-Day Freshness Check for {len(targets)} ASNs...")
    ...
    print(f"\n[*] Harvest Complete. {stats['updated']} records refreshed.")
