...
def load_data():
    ...
    # 1. Metadata with Strict Validation
    meta_map = {}
    if os.path.exists(FILE_ASNS_CSV):
        try:
            ...
            for _, row in df.iterrows():
                # Validate ASN
                raw_asn = row.get('asn')
                if pd.isna(raw_asn): continue
                
                s = str(raw_asn).upper().replace('AS','')
                if not s.isdigit(): continue
                
                # Validate Name/CC
                name = str(row.get('name','Unknown')).strip()
                cc = str(row.get('country','XX')).upper().strip()
                
                # Corruption Check: If Name is 2 chars uppercase (likely a CC) and CC is weird
                if len(name) == 2 and name.isupper() and (len(cc) > 2 or cc.isdigit()):
                    # Likely a shifted row in source. Skip or Fix?
                    # Let's try to assume Name is actually the CC, and Name is Unknown
                    cc = name
                    name = "Unknown (Data Error)"

                meta_map[int(s)] = {'name': name, 'cc': cc}
...
def analyze():
    ...
    # --- SANITIZATION PASS (NEW) ---
    print(f"[*] Post-Processing: {len(df)} rows raw.")
    
    # Drop rows where ASN is 0 or Null
    df = df[df['asn'] > 0]
    
    # Drop rows where Name matches country code regex (2 chars, uppercase) AND CC is numeric/weird
    # This catches the ",AR,24" shift
    # Condition: Name len is 2 AND Name is Upper AND CC is NOT 2-char-alpha
    mask_corrupt = (df['name'].str.len() == 2) & df['name'].str.isupper() & (~df['cc'].str.isalpha())
    if mask_corrupt.any():
        print(f"    - Dropped {mask_corrupt.sum()} corrupted rows (Column Shift detected).")
        df = df[~mask_corrupt]

    print(f"    - Final Clean Count: {len(df)} rows.")

    # --- REPORT ---
    print("\n" + "="*80)
    print("GLOBAL ROV AUDIT REPORT (V11 - SANITIZED)")
    print("="*80)
    ...
    # Top 50 Known Unknowns
    print("\n" + "="*80)
    print("TOP 50 KNOWN UNKNOWNS (Missing Graph Data)")
    print("="*80)
